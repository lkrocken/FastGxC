...
#' @param  output_file_name - full file path of output file where eQTL files will be written out
#' @return outputs one file of mapped cis eQTLs
#'
#' @export
eQTL_mapping_step = function(SNP_file_name, snps_location_file_name, expression_file_name, gene_location_file_name, output_file_name){

# Output file name
output_file_name_cis = output_file_name; 
output_file_name_tra = tempfile();

setDTthreads(1)
print(paste0("data.table getDTthreads(): ",getDTthreads()))

sprintf("Running analysis for %s", expression_file_name)
